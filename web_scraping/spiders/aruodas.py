...
class AruodasSpider(scrapy.Spider):
    ...
    def parse_project(self, r):
        ...
        obj_type_id, advert_id, _, _, project_id, _, project_owner = r.css('script::text').re_first(js_pattern).split(', ')
        params = {
            'obj': obj_type_id,
            'id': advert_id,
        ...
        yield r.follow(f'/ajax/getProjectAdverts/?{params_str}', callback=self.parse_project_variants)
    ...
    def parse_ad(self, r):
        ...
        project_developer_link = r.css('.projects-popup__developer__url a::attr(href)').get() if project_name_raw else None
